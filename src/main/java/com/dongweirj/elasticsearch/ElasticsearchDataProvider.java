package com.dongweirj.elasticsearch;
...
import com.dongweirj.dataprovider.config.CompositeConfig;
import com.dongweirj.dataprovider.config.ConfigComponent;
import com.dongweirj.dataprovider.config.DimensionConfig;
import com.dongweirj.dataprovider.config.ValueConfig;
import com.dongweirj.elasticsearch.aggregation.AggregationBuilder;
import com.dongweirj.elasticsearch.query.QueryBuilder;
import com.dongweirj.util.SqlMethod;
import com.dongweirj.util.json.JSONBuilder;
...
import com.dongweirj.cache.CacheManager;
import com.dongweirj.cache.HeapCacheManager;
import com.dongweirj.dataprovider.DataProvider;
import com.dongweirj.dataprovider.Initializing;
import com.dongweirj.dataprovider.aggregator.Aggregatable;
import com.dongweirj.dataprovider.annotation.DatasourceParameter;
import com.dongweirj.dataprovider.annotation.ProviderName;
import com.dongweirj.dataprovider.annotation.QueryParameter;
import com.dongweirj.dataprovider.config.*;
import com.dongweirj.dataprovider.result.AggregateResult;
import com.dongweirj.dataprovider.result.ColumnIndex;
...
import static com.dongweirj.elasticsearch.aggregation.AggregationBuilder.dateHistAggregation;

/**
 * Created by dw on 2017/3/17.
 */
...
public class ElasticsearchDataProvider extends DataProvider implements Aggregatable, Initializing {
    ...
    private JSONObject configComponentToFilter(ConfigComponent cc) {
        if (cc instanceof DimensionConfig) {
            ...
            QueryBuilder.BoolType boolType = QueryBuilder.BoolType.MUST;
            if ("AND".equalsIgnoreCase(compositeConfig.getType())) {
                boolType = QueryBuilder.BoolType.MUST;
            } else if ("OR".equalsIgnoreCase(compositeConfig.getType())) {
                boolType = QueryBuilder.BoolType.SHOULD;
            }
    ...
    private JSONObject getFilterPart(DimensionConfig config) {
        ...
        if (NULL_STRING.equals(v0)) {
            switch (config.getFilterType()) {
                case "=":
                case "≠":
                    return QueryBuilder.nullQuery(fieldName, "=".equals(config.getFilterType()));
            }
        }
        switch (config.getFilterType()) {
            case "=":
            case "eq":
                return QueryBuilder.termsQuery(fieldName, config.getValues());
            case "≠":
            case "ne":
                return getFilterPartEq(QueryBuilder.BoolType.MUST_NOT, fieldName, config.getValues());
            case ">":
                return QueryBuilder.rangeQuery(fieldName, v0, null);
            case "<":
                return QueryBuilder.rangeQuery(fieldName, null, v0);
            case "≥":
                return QueryBuilder.rangeQuery(fieldName, v0, null, true, true);
            case "≤":
                return QueryBuilder.rangeQuery(fieldName, null, v0, true, true);
            case "(a,b]":
                return QueryBuilder.rangeQuery(fieldName, v0, v1, false, true);
            case "[a,b)":
                return QueryBuilder.rangeQuery(fieldName, v0, v1, true, false);
            case "(a,b)":
                return QueryBuilder.rangeQuery(fieldName, v0, v1, false, false);
            case "[a,b]":
                return QueryBuilder.rangeQuery(fieldName, v0, v1, true, true);
        }
        return null;
    }

    private JSONObject getFilterPartEq(QueryBuilder.BoolType boolType, String fieldName, List<String> values) {
        JSONArray boolArr = new JSONArray();
        values.stream()
                .map(e -> QueryBuilder.termQuery(fieldName, e))
                .forEach(boolArr::add);
        return QueryBuilder.boolFilter(boolType, boolArr);
    }
        ...
        try {
            ...
            switch (types.get(d.getColumnName())) {
                ...
                default:
                    ...
                    aggregation = JSONBuilder.json(d.getColumnName(), AggregationBuilder
                            .termsAggregation(d.getColumnName(), 1000, missing));
            }
            // Query Override
            if (overrideAgg != null) {
    ...
    private JSONObject buildDateHistAggregation(String columnName, AggConfig config) throws Exception {
        ...
        Long lower = array.stream()
                .map(jo -> SqlMethod.coalesce(jo.getLong("gt"), jo.getLong("gte")))
                .filter(Objects::nonNull)
                .max(Comparator.naturalOrder())
                .orElse(null);
        Long upper = array.stream()
                .map(jo -> SqlMethod.coalesce(jo.getLong("lt"), jo.getLong("lte")))
                .filter(Objects::nonNull)
        ...
        return JSONBuilder.json(columnName, AggregationBuilder
                .dateHistAggregation(columnName, intervalStr, 0, lower, upper));
    }

    private JSONObject queryBound(String columnName, AggConfig config) {
        String maxKey = "max_ts";
        String minKey = "min_ts";
        JSONBuilder request = JSONBuilder.json("size", 0).
                put("aggregations", JSONBuilder.json().
                        put(minKey, JSONBuilder.json("min", JSONBuilder.json("field", columnName))).
                        put(maxKey, JSONBuilder.json("max", JSONBuilder.json("field", columnName)))
                );
        ...
        try {
            JSONObject response = post(getSearchUrl(request), request);
            long maxTs = SqlMethod
                    .coalesce(response.getJSONObject("aggregations").getJSONObject(maxKey).getLong("value"), 0l);
            long minTs = SqlMethod
                    .coalesce(response.getJSONObject("aggregations").getJSONObject(minKey).getLong("value"), 0l);
            intervalStr = dateInterval(minTs, maxTs);
        } catch (Exception e) {
            LOG.error("", e);
        }
        return JSONBuilder
                .json(columnName, AggregationBuilder.dateHistAggregation(columnName, intervalStr, 0));
    }
    ...
    public JSONObject buildFilterDSL(AggConfig config) {
        return QueryBuilder.boolFilter(QueryBuilder.BoolType.FILTER, getFilter(config));
    }
